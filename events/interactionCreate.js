...
return interaction?.reply({ content: "Rate terbatas", ephemeral: true })
} else {
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: 'Rate terbatas'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `lah u ga masuk voice, ogah!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `masuk voice dlu kocak!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Missing Permissions: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "Kirim Pesan"}**`, ephemeral: true });
            }
            } catch (e) {
            return interaction?.reply({ content: `nahkan Error...\n\n\`\`\`${e?.message}\`\`\``, ephemeral: true });
            }
            }
            });
            });
