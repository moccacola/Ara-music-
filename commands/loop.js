...
module.exports = {
  name: "loop",
  description: "aktifin mode pengulangan lagu",
    ...
    try {
      ...
      let button = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setCustomId("queue"),
        new ButtonBuilder()
          .setLabel("Lagu Sekarang")
          .setStyle(ButtonStyle.Secondary)
          .setCustomId("nowplaying"),
        new ButtonBuilder()
          .setLabel("Matikan Loop!")
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('**puter ulang terus smpe budeg. sampe earwarm aku ga tanggung jawab!**')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: 'gada musiknya coy!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `putar ulang antrian!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `mode looping aktif!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: 'mode looping dimatikan!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: 'mode looping mati' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
