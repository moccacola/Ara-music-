...
module.exports = {
  name: "queue",
  description: "nampilin daftar antrian",
  ...
  run: async (client, interaction) => {
    ...
    try {
     ...
     const queue = client.player.getQueue(interaction.guild.id);
      ...
      if (!queue.songs[0]) return interaction.reply({ content: 'gada antrian coy!!', ephemeral: true }).catch(e => { })
      ...
      const generateEmbed = async (start) => {
        ...
        if (!current || !current?.length > 0) return interaction.reply({ content: 'Gada antrian coy!!', ephemeral: true }).catch(e => { })
        return new EmbedBuilder()
          .setTitle(`${interaction.guild.name}  Queue`)
          .setThumbnail(interaction.guild.iconURL({ size: 2048, dynamic: true }))
          .setColor(client.config.embedColor)
          .setDescription(`Sekarang muter: \`${queue.songs[0].name}\`
    ${current.map(data =>
      ...
      await interaction.reply({
        ...
        collector.on("collect", async (button) => {
          if (button?.customId === "close") {
            collector?.stop()
           return button?.reply({ content: 'cmd dibatalin', ephemeral: true }).catch(e => { })
          } else {
        ...
        collector.on("end", async (button) => {
          ...
          const embed = new EmbedBuilder()
            ...
            .setColor(`#ecfc03`)
            .setDescription('coba pencet lagi tombolnya!!')
